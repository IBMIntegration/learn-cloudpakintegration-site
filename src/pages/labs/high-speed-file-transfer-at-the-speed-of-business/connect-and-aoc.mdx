...
You are now familiar with two methods of initiating an Aspera transfer; using the CLI and
using the native Aspera Client. Another option is to use the `Aspera Connect` client
application.

Aspera Connect is designed to allow easy FASP uploads and downloads using the browser.
Lets now use the Aspera Connect Client to download the file that contains the token we
need for the next section of this lab.

## Access Aspera on Cloud
...
1. The Aspera Connect application will open and display the progress of the transfer
   ...
   **Note:** If the Aspera Connect Client window is not focussed you can select the icon
   ...
   Once complete the file `lab5352_secret.txt` will be available in `/home/ibmuser`. You
   will use this in the next section.

## Recap

Aspera on Cloud makes it easy for you to get access to your files no matter where they are
located in the world. In this section of the tutorial you used the Aspera Connect Client
to download a file hosted in AWS S3 Toronto using a browser based client.

Now you have that token you can go back and use the Aspera API to initiate a remote
transfer between the Kubernetes environment and Aspera on Cloud.
